...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/github_bridge.log'),
...
class GitHubIssuesBridge:
    ...
    def get_issue_details(self, issue_number):
        ...
        if response:
            ...
            # Get comments
            comments_response = self._make_request('GET', f"{self.base_url}/issues/{issue_number}/comments")
    ...
    def add_comment(self, issue_number, body):
        ...
        data = {'body': body}
        response = self._make_request('POST', f"{self.base_url}/issues/{issue_number}/comments", json=data)
        if response:
    ...
    def create_branch(self, branch_name, from_branch='main'):
        ...
        try:
            ...
            subprocess.run(['git', 'fetch', 'origin'], check=True, cwd='/workspaces/letterboxd-friend-check')
            subprocess.run(['git', 'checkout', from_branch], check=True, cwd='/workspaces/letterboxd-friend-check')
            subprocess.run(['git', 'pull', 'origin', from_branch], check=True, cwd='/workspaces/letterboxd-friend-check')
            
            # Create and checkout new branch
            subprocess.run(['git', 'checkout', '-b', branch_name], check=True, cwd='/workspaces/letterboxd-friend-check')
    ...
    def commit_changes(self, message, files=None):
        ...
        try:
            if files:
                for file in files:
                    subprocess.run(['git', 'add', file], check=True, cwd='/workspaces/letterboxd-friend-check')
            else:
                subprocess.run(['git', 'add', '.'], check=True, cwd='/workspaces/letterboxd-friend-check')
            
            subprocess.run(['git', 'commit', '-m', message], check=True, cwd='/workspaces/letterboxd-friend-check')
    ...
    def push_branch(self, branch_name):
        ...
        try:
            subprocess.run(['git', 'push', 'origin', branch_name], check=True, cwd='/workspaces/letterboxd-friend-check')
    ...
    def generate_issue_report(self, state='open'):
        ...
        for issue in issues:
            ...
            # Count by component
            for component in analysis['components']:
                report['issues_by_component'][component] = report['issues_by_component'].get(component, 0) + 1
...
def main():
    ...
    parser.add_argument('--create-issue', nargs=2, metavar=('TITLE', 'BODY'), help='Create new issue')
    
    # Options
    parser.add_argument('--state', choices=['open', 'closed', 'all'], default='open', help='Issue state')
